...
const BrandLoader: React.FC<{ size?: string; animate?: boolean; grayscale?: boolean }> = ({ size = "w-24 h-24", animate = true, grayscale = false }) => (
  <div className={`${size} relative ${animate ? 'animate-bounce' : ''} ${grayscale ? 'grayscale opacity-30' : ''}`}>
    <div className={`absolute inset-0 bg-gradient-to-br from-[#BD1023] to-[#8E0C1B] rounded-full shadow-2xl border-4 border-white`}></div>
      ...
      {showCamera && (
        <div className="fixed inset-0 z-[200] bg-black/90 flex flex-col items-center justify-center p-4 backdrop-blur-sm">
          <div className="relative w-full max-w-lg aspect-[3/4] rounded-2xl overflow-hidden border-2 border-white/20">
            ...
            <div className="absolute top-0 left-0 w-full h-1 bg-[#BD1023] shadow-[0_0_15px_#BD1023] animate-[scan_3s_linear_infinite]" />
          </div>
          <div className="flex gap-6 mt-8">
            ...
            <button onClick={captureImage} className="w-16 h-16 bg-[#BD1023] rounded-full border-4 border-white flex items-center justify-center text-white shadow-xl">
              <CameraIcon />
            </button>
          </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 animate-in fade-in slide-in-from-bottom-6 duration-700">
          <div className="lg:col-span-8 space-y-8">
            <div className="bg-white rounded-2xl p-10 border border-[#f0ede5] shadow-xl relative overflow-hidden group">
                <div className="flex justify-between items-start relative z-10">
                    <div className="flex-1">
                        <div className="flex items-baseline gap-6 mb-4">
                            <h1 className="text-8xl font-black chinese-font text-[#1A1A1A] tracking-tight leading-none">{result.simplified}</h1>
                            <div className="flex flex-col gap-2">
                                {result.hskLevel && <span className="px-3 py-1 bg-[#1A1A1A] text-white text-[9px] font-black rounded-lg uppercase tracking-widest w-fit">HSK {result.hskLevel}</span>}
                                <button 
                                  onClick={() => copyToClipboard(result.simplified)}
                                  className="text-[9px] font-black text-[#BD1023] uppercase tracking-[0.3em] hover:underline text-left"
                                >
                                  {t('copyChar')}
                                </button>
                            </div>
                        </div>
                        <p className="text-3xl font-black text-[#BD1023] tracking-[0.3em] mb-10 uppercase">{result.pinyin}</p>
                        
                        <div className="space-y-4">
                            {result.meanings.map((m, i) => (
                                <div key={i} className="flex gap-4 items-start group">
                                    <span className="text-gray-200 font-black italic text-xl min-w-[1.5rem]">{i+1}</span>
                                    <p className="text-xl text-[#4A4A4A] leading-relaxed font-medium group-hover:text-[#1A1A1A] transition-colors">{m}</p>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            <div className="bg-white rounded-2xl p-10 border border-[#f0ede5] shadow-sm">
                <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-8">{t('usageExamples')}</h3>
                <div className="space-y-10">
                    {result.examples.map((ex, i) => (
                        <div key={i} className="group cursor-pointer border-l-4 border-transparent hover:border-[#BD1023] pl-6 transition-all">
                            <p className="text-2xl font-black text-[#1A1A1A] mb-2 chinese-font leading-relaxed tracking-wide group-hover:text-[#BD1023] transition-colors">
                              {ex.chinese}
                            </p>
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="bg-[#f9f7f2] rounded-2xl p-8 border border-[#f0ede5]">
                ...
                <div className="grid grid-cols-2 gap-4">
                    {result.components.map((c, i) => (
                        <button 
                          ...
                          className="bg-white p-5 rounded-xl shadow-sm text-center hover:shadow-xl hover:-translate-y-1.5 transition-all border border-[#f0ede5] group"
                        >
                            <div className="text-3xl font-black chinese-font text-[#1A1A1A] mb-2 group-hover:text-[#BD1023]">{c.char}</div>
                            <div className="text-[9px] font-black text-gray-400 uppercase tracking-widest">{c.meaning}</div>
                            {c.radical && <div className="mt-2 text-[8px] font-black text-[#BD1023] uppercase tracking-widest">{t('radical')}</div>}
                        </button>
                    ))}
                </div>
            </div>

            <div className="bg-white rounded-2xl p-8 border border-[#f0ede5] shadow-sm">
                ...
                <div className="space-y-3">
                    {result.compounds.map((comp, i) => (
                        <button 
                          ...
                          className="w-full text-left p-4 rounded-xl hover:bg-[#fdf0f1] hover:border-[#fae2e4] border border-transparent transition-all group"
                        >
                            <div className="text-xl font-black chinese-font text-[#1A1A1A] mb-0.5 group-hover:text-[#BD1023]">{comp.word}</div>
