...
const BrandLogo: React.FC<{ size?: string }> = ({ size = "w-12 h-12" }) => (
  <div className={`${size} relative animate-in zoom-in duration-500 group shrink-0`}>
    <div className="absolute inset-0 bg-gradient-to-br from-[#BD1023] to-[#8E0C1B] rounded-xl shadow-lg transform transition-all group-hover:scale-110 group-hover:rotate-3"></div>
...
const Navigation: React.FC<NavigationProps> = ({ currentView, setView }) => {
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-[100] glass border-t border-[#f0ede5] pb-[env(safe-area-inset-bottom)]">
        <div className="flex justify-around items-center h-16">
          {items.map(({ id, label, Icon }) => (
            <button key={id} onClick={() => setView(id)} className="flex flex-col items-center justify-center flex-1 transition-all">
              ...
              <span className={`text-[9px] font-black uppercase mt-1.5 tracking-widest ${currentView === id ? 'text-[#BD1023]' : 'text-gray-400'}`}>{label}</span>
            </button>
          ))}
      ...
      <nav className="hidden md:flex flex-col w-64 h-screen bg-white border-r border-[#f0ede5] sticky top-0 overflow-y-auto">
        <div className="p-6 lg:p-8 flex flex-col h-full">
          <div className="flex items-center gap-4 mb-12 cursor-pointer group" onClick={() => setView(View.DASHBOARD)}>
            <BrandLogo />
            <div>
              <h1 className="text-2xl font-black tracking-tighter text-[#1A1A1A] leading-none">Hanzi<span className="text-[#BD1023] italic">Go</span></h1>
          ...
          <div className="space-y-1.5 flex-1">
            {items.map(({ id, label, Icon }) => (
              <button key={id} onClick={() => setView(id)} className={`flex items-center gap-4 w-full px-5 py-3 rounded-xl transition-all font-bold text-sm ${currentView === id ? 'bg-[#fdf0f1] text-[#BD1023] shadow-sm' : 'text-gray-500 hover:bg-[#f9f7f2]'}`}>
                <Icon active={currentView === id} />
                <span>{label}</span>
              </button>
            ))}
